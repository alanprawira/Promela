...
short src_ln0 [] = {
	  0,  22,  25,  28,  31,  32,  33,  35, 
	 30,  37,  37,  38,  39,  27,  42,  42, 
	 45,  47,  48,  49,  50,  44,  52,  24, 
	 53,  24,  53,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "leader.pml", 1, 26 },
...
{	int j = 0, h = now._nr_pr;
	...
	case 0:	/* Node */
		...
		logval("Node:maxid", ((P0 *)pptr(h))->maxid);
		logval("Node:action", ((P0 *)pptr(h))->action);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.LtoR[0] = addqueue(calling_pid, 1, 1);
		now.LtoR[1] = addqueue(calling_pid, 2, 1);
		now.LtoR[2] = addqueue(calling_pid, 3, 1);
		now.LtoR[3] = addqueue(calling_pid, 4, 1);
		now.RtoL[0] = addqueue(calling_pid, 5, 1);
		now.RtoL[1] = addqueue(calling_pid, 6, 1);
		now.RtoL[2] = addqueue(calling_pid, 7, 1);
		now.RtoL[3] = addqueue(calling_pid, 8, 1);
#ifdef VAR_RANGES
#endif
}
...
#endif
	switch (n) {
	case 8: j = sizeof(Q8); q_flds[8] = 2; q_max[8] = 1; break;
	case 7: j = sizeof(Q7); q_flds[7] = 2; q_max[7] = 1; break;
	case 6: j = sizeof(Q6); q_flds[6] = 2; q_max[6] = 1; break;
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.LtoR[0]) )
			 ||     q_full((int) ( now.LtoR[1]) )
			 ||     q_full((int) ( now.LtoR[2]) )
			 ||     q_full((int) ( now.LtoR[3]) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.LtoR[0]) )
			 ||    !q_full((int) ( now.LtoR[1]) )
			 ||    !q_full((int) ( now.LtoR[2]) )
			 ||    !q_full((int) ( now.LtoR[3]) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.LtoR[0]) )
			 ||     !q_len((int) ( now.LtoR[1]) )
			 ||     !q_len((int) ( now.LtoR[2]) )
			 ||     !q_len((int) ( now.LtoR[3]) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.LtoR[0]) )
			 ||      q_len((int) ( now.LtoR[1]) )
			 ||      q_len((int) ( now.LtoR[2]) )
			 ||      q_len((int) ( now.LtoR[3]) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.RtoL[0]) )
			 ||     q_full((int) ( now.RtoL[1]) )
			 ||     q_full((int) ( now.RtoL[2]) )
			 ||     q_full((int) ( now.RtoL[3]) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.RtoL[0]) )
			 ||    !q_full((int) ( now.RtoL[1]) )
			 ||    !q_full((int) ( now.RtoL[2]) )
			 ||    !q_full((int) ( now.RtoL[3]) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.RtoL[0]) )
			 ||     !q_len((int) ( now.RtoL[1]) )
			 ||     !q_len((int) ( now.RtoL[2]) )
			 ||     !q_len((int) ( now.RtoL[3]) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.RtoL[0]) )
			 ||      q_len((int) ( now.RtoL[1]) )
			 ||      q_len((int) ( now.RtoL[2]) )
			 ||      q_len((int) ( now.RtoL[3]) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  token:	3\n");
	printf("	mtype  elect:	2\n");
	printf("	mtype  leader:	1\n");
	printf("	chan LtoR[0] (=%d):	len %d:\t", now.LtoR[0], q_len(now.LtoR[0]));
	c_chandump(now.LtoR[0]);
	printf("	chan LtoR[1] (=%d):	len %d:\t", now.LtoR[1], q_len(now.LtoR[1]));
	c_chandump(now.LtoR[1]);
	printf("	chan LtoR[2] (=%d):	len %d:\t", now.LtoR[2], q_len(now.LtoR[2]));
	c_chandump(now.LtoR[2]);
	printf("	chan LtoR[3] (=%d):	len %d:\t", now.LtoR[3], q_len(now.LtoR[3]));
	c_chandump(now.LtoR[3]);
	printf("	chan RtoL[0] (=%d):	len %d:\t", now.RtoL[0], q_len(now.RtoL[0]));
	c_chandump(now.RtoL[0]);
	printf("	chan RtoL[1] (=%d):	len %d:\t", now.RtoL[1], q_len(now.RtoL[1]));
	c_chandump(now.RtoL[1]);
	printf("	chan RtoL[2] (=%d):	len %d:\t", now.RtoL[2], q_len(now.RtoL[2]));
	c_chandump(now.RtoL[2]);
	printf("	chan RtoL[3] (=%d):	len %d:\t", now.RtoL[3], q_len(now.RtoL[3]));
	c_chandump(now.RtoL[3]);
}
void
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("leader"); return;
	case 2: Printf("elect"); return;
	case 3: Printf("token"); return;
